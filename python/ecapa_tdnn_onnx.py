...
"""
ECAPA-TDNN ONNX Export Script

This script exports the ECAPA-TDNN speaker embedding model to ONNX format.
Note: The exported model expects mel-spectrogram features as input, not raw audio.

Input format: (batch_size, time_frames, mel_features)
- batch_size: number of audio samples
- time_frames: number of time frames (variable)
- mel_features: 80 mel-spectrogram features per frame

Output format: (batch_size, 1, embedding_dim)
- embedding_dim: 192-dimensional speaker embedding

Usage:
1. Run this script to export the model: python ecapa_tdnn_onnx.py
2. Use the exported model with ONNX Runtime in your application
3. Preprocess audio to mel-spectrogram features before inference

Note: Feature extraction (raw audio -> mel-spectrogram) must be done separately
as STFT operations are not supported in ONNX export.
"""
...
# Create dummy mel-spectrogram features (batch, time, features)
# This represents the output of compute_features
dummy_features = torch.randn(1, 200, 80)  # (batch_size=1, time=200, features=80)
...
class ECAPAWrapper(torch.nn.Module):
    ...
    def forward(self, features):
        # ECAPA-TDNN expects (batch, time, features) format
...
print("✅ Exported to ecapa_tdnn_small.onnx")
